...
def edit_profile_admin(id):
    user = User.query.get_or_404(user=User)
...
def follow(username):
    ...
    db.session.commit()
    flash('now following %s' % username)
    return redirect(url_for('.user', username=username))
...
def followers(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followers.paginate(page=page, per_page=current_app.config['FOLLOWERS_PER_PAGE'], error_out=False)
...
def followed_by(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagination = user.followed.paginate(page=page, per_page=current_app.config['FOLLOWERS_PER_PAGE'], error_out=False)
