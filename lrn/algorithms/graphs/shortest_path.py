...
def shortest_path(g, origin, destination, column_name='cost'):
    if g.vertices.filter(g.vertices.id == destination).count() == 0:
        return spark.createDataFrame(None, g.vertices.schema).withColumn('path', F.array())
    vertices = g.vertices.withColumn('visited', F.lit(False)).withColumn('distance', F.when(g.vertices['id'] == origin,
                                                                                            0)).otherwise(
        float('inf')).withColumn('path', F.array())
    ...
    while g2.vertices.filter('visited == False').first():
        ...
        new_path_col = F.when(
            new_distances['addMess'].isNotNull() & (new_distances.aggMess['col1'] < g2.vertices.distance),
            new_distances.aggMess['col2'].cast('array<string>')).otherwise(g2.vertices.path)
        new_vertices = g2.vertices.join(new_distances, on='id', how='left_outer').drop(new_distances['id']).withColumn(
            'visited', new_visited_col).withColumn('newDistance', new_distance_col).withColumn('newPath',
                                                                                               new_path_col).drop(
            'aggMess', 'distance', 'path').withColumnRenamed('newDistance', 'distance').withColumnRenamed('newPath',
                                                                                                          'path')
        cached_new_vertices = AM.getCachedDataFrame(new_vertices)
        g2 = GraphFrame(cached_new_vertices, g2.edges)
        if g2.vertices.filter(g2.vertices.id == destination).first().visited:
            return g2.vertices.filter(g2.vertices.id == destination).withColumn('newPath',
                                                                                add_path_udf('path', 'id')).drop(
                'visited', 'path').withColumnRenamed('newPath', path)
    return spark.createDataFrame(None, g.vertices.schema).withColumn('path', F.array())
